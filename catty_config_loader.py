...
def _apply_config(data: dict[str, Any], base_dir: Path) -> None:
    ...
    # CPU 主回复引擎 (BotLibre 五层) + 积分门控 + 强互动 + 每日 DeepSeek 进化.
    # 默认 enabled=False, 关停时透传到现 keyword_reply / handle_chat 链路.
    cpu_engine = _section(data, "cpu_engine")
    _set_env("CATTY_CPU_ENGINE_ENABLED", cpu_engine.get("enabled"))
    _set_env("CATTY_CPU_ENGINE_ROUTES_DIR", cpu_engine.get("routes_dir"))
    _set_env("CATTY_CPU_ENGINE_CORPUS_PATH", cpu_engine.get("corpus_path"))
    _set_env("CATTY_CPU_ENGINE_TXTAI_INDEX_PATH", cpu_engine.get("txtai_index_path"))
    _set_env("CATTY_CPU_ENGINE_L2_THRESHOLD_DIRECT", cpu_engine.get("l2_threshold_direct"))
    _set_env("CATTY_CPU_ENGINE_L2_THRESHOLD_CANDIDATE", cpu_engine.get("l2_threshold_candidate"))
    _set_env("CATTY_CPU_ENGINE_GROUP_THRESHOLD_BONUS", cpu_engine.get("group_threshold_bonus"))
    _set_env("CATTY_CPU_ENGINE_L4_ENABLED_PRIVATE", cpu_engine.get("l4_enabled_private"))
    ...
    credit = _section(data, "credit")
    _set_env("CATTY_CREDIT_ENABLED", credit.get("enabled"))
    _set_env("CATTY_CREDIT_INITIAL_BALANCE", credit.get("initial_balance"))
    _set_env("CATTY_CREDIT_DAILY_SIGNIN_AMOUNT", credit.get("daily_signin_amount"))
    _set_env("CATTY_CREDIT_WEEKLY_SIGNIN_BONUS", credit.get("weekly_signin_bonus"))
    _set_env("CATTY_CREDIT_PASSIVE_RECOVER_PER_HOUR", credit.get("passive_recover_per_hour"))
    _set_env("CATTY_CREDIT_PASSIVE_RECOVER_CAP", credit.get("passive_recover_cap"))
    _set_env("CATTY_CREDIT_DEEPSEEK_BASE_COST", credit.get("deepseek_base_cost"))
    _set_env("CATTY_CREDIT_DEEPSEEK_PER_1K_PROMPT", credit.get("deepseek_per_1k_prompt"))
    _set_env("CATTY_CREDIT_DEEPSEEK_PER_1K_COMPLETION", credit.get("deepseek_per_1k_completion"))
    ...
    _set_env("CATTY_STRONG_INTENTS", strong.get("intents"), json_value=True)
    _set_env("CATTY_STRONG_NSFW_PHASE_THRESHOLD", strong.get("nsfw_phase_threshold"))

    evolution = _section(data, "evolution")
    _set_env("CATTY_EVOLUTION_ENABLED", evolution.get("enabled"))
    _set_env("CATTY_EVOLUTION_CRON", evolution.get("cron"))
    _set_env("CATTY_EVOLUTION_SAMPLES_PER_LAYER", evolution.get("samples_per_layer"))
    _set_env("CATTY_EVOLUTION_JUDGE_MODEL", evolution.get("judge_model"))
    _set_env("CATTY_EVOLUTION_ROLLBACK_NEG_FEEDBACK_PCT", evolution.get("rollback_neg_feedback_pct"))
    _set_env("CATTY_EVOLUTION_ROLLBACK_SCORE_DECLINE_DAYS", evolution.get("rollback_score_decline_days"))
    _set_env("CATTY_EVOLUTION_SAMPLE_ONLY_GROUP", evolution.get("sample_only_group"))
    _set_env("CATTY_EVOLUTION_LOGS_DIR", evolution.get("logs_dir"))
    _set_env("CATTY_EVOLUTION_GIT_COMMIT_ENABLED", evolution.get("git_commit_enabled"))
