...
def build_action_hints(
    ...
    *,
    ...
    内部会调 classify_intent + extract_entities;为简洁省去 caller 重复传参。
    如果以后 caller 已经有结果想复用,可以再加 overload。
    """
    if not text:
        return ""
    ...
    if future and {"command_to_cat", "question"} & set(intent_tags):
        # 引用 iso 让建议更精准
        hints.append(
    ...
    if _has_kind(entities, "url") and {"info_request", "question"} & set(intent_tags):
        hints.append(
    ...
    # 4) 金额 + 命令/问句 → 提醒精确
    if _has_kind(entities, "money") and {"command_to_cat", "question"} & set(intent_tags):
        hints.append(
            "消息涉及金额数字,如果你要复述或答应什么,**数字必须原样准确**,不要口误"
        )

    # 5) 冷场 + 招呼 → 主动开话题
    if pulse_phase == "cold" and "greeting" in intent_tags:
        hints.append(
    ...
    # 6) 刷屏 + 撩猫 → 短反应不陪刷
    if pulse_phase == "burst" and "tease_cat" in intent_tags:
        hints.append(
