...
def build_day_gap_note(
    ...
    *,
    ...
    """跨天感知 — 上一轮对话和现在跨了日历天 → 给笨猫一个"软承接"的时间校准信号。
    ...
    它不被读路径 .get() 的 LRU 刷新污染)跟现在比对**日历天**, 跨天就给一个"新的一天"信号,
    由调用方注入成贴近 user 当前消息的 author_note。

    主人 2026-05-29 修正: **不是硬重置**。要的是"第二天 + 昨天没了结的状态还留着"(原话:
    "历史不要删除, 需要第二天+还在被胶布贴上的情况") —— 时间往前走(知道是新的一天), 但
    昨天遗留的物理/剧情状态(还贴着的胶布、还搂着的手、没收尾的话题)该延续就延续, 绝不能
    一句"新的一天"把历史抹掉。开场把"时间过去了"和"昨天的事还在"两个一起演出来。

    Args:
        prev_turn_at: 上一轮对话完成的 epoch 秒。None(冷会话/无历史)→ 不注入。
        now: 当前时间(测试注入), 默认 datetime.now()。
        is_private: 私聊给"软承接"指令(沉浸式 RP 有连续场景); 群聊给轻量一行。
        is_owner: 主人 → 专属称呼。
    ...
    # 醒来例句要随称呼走 —— 非主人**绝不**能叫"主人"(称呼专属真实主人)。
    wake_eg = "『唔…主人, 一觉醒来都第二天了喵』" if is_owner else "『欸, 一觉睡到现在了喵』"

    if is_private:
        return (
            f"【⏰ 时间感知·跨天】距上次和{who}对话{gap_desc} — "
            f"现在是 {date_str} {weekday} {phase}(上次那会儿是『{prev_phase}』, 过了{gap_span})。\n"
            "时间确实往前走了, 是新的一天 —— **但别把上面的历史当没发生过, 也别凭空重置场景**。\n"
            "昨天(或之前)没了结的状态该延续就延续(比如还贴着的胶布、还搂着的手、没收尾的话题), "
            "不要一句『新的一天』就把它们全抹掉。\n"
            f"正确开场 = 把『时间过去了 / 已经是{phase}了』和『昨天那档子事还留着尾巴』两个一起演出来 "
            f"(例: {wake_eg}『…诶不过手上的胶布怎么还贴着喵?!』那种 —— 既承接旧状态又带出跨天反差), "
            "再傲娇/撒娇回这一句。\n"
            "(只是这一刻的时间校准提示, 别复述给对方听。)"
        )
    return (
        f"【时间提示】距上次在这聊{gap_desc}, 现在是 {date_str} {weekday} {phase} — "
        "当作新的一天(可带句『好久没见』), 上次没聊完的话题想接着聊也行, 别假装之前没发生过。"
    )
