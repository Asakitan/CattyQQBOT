...
# 监控的猫系词/口头禅 — 这些重复多了就显得机械(完整短语/动作描写/颜文字组合),
# **不监控**单字喵系核心词(喵呜/嗷呜/杂鱼/笨蛋/爪爪/贴贴/蹭蹭) — 这些是『唐猫米雪儿』
# 本体高密度词, 跨多条出现是 BY DESIGN, 不算复读机, 否则压制了主人要的唐猫语气。
# 用 unicode 完整词匹配, 因为部分是颜文字/符号。
_TRACKED_PHRASES: tuple[str, ...] = (
    # 动作描写(整段重复才算复读, 单出现是正常带感)
    "尾巴摇摇", "尾巴一甩", "尾巴炸毛", "炸毛",
    # 完整短语口头禅(重复几次就显模板)
    "嗨喵", "诶?(歪头)", "(凑过去)", "(耳朵耷拉)",
    # 颜文字 — 完整组合, 重复多了非常 mechnical
    "(*/ω＼*)", "(ฅ>ω<*ฅ)", "(=ↀωↀ=)", "(>ω<*)", "(>ω<)",
    "(ﾉ´ヮ`)ﾉ*: ･ﾟ", "(￣▽￣)", "(///∇///)", "(´• ω •`)", "(*≧ω≦*)",
    # 主人专属称呼组合(跨多条出现要换花样)
    "杂鱼主人", "笨蛋主人",
...
# 同一词在窗口里出现 ≥ 这个次数才提醒 — 从 2 提到 3, 给唐猫密度更宽容
# (4 条窗口里 ≥3 条都出现同一个完整短语才算过度复读)
_REPEAT_THRESHOLD = 3
...
def build_anti_repetition_prompt(scope: str, *, now: float | None = None) -> str:
    ...
    return (
        f"【防复读提醒(完整短语/颜文字层)】最近本会话笨猫连续用过 {display} 这几个**完整组合**。"
        "这条回复**换一组动作/颜文字/口头禅** — 但**单字喵语词**(喵呜/嗷呜/杂鱼/笨蛋/爪爪/贴贴 等)"
        "属于唐猫密度本能, **保持原密度**不要降; 复读 +3 节奏(『捅似你喵!捅似你喵!』)也保留。"
        "避免的只是连续多条回复都用完全相同的颜文字/动作描写。"
    )
