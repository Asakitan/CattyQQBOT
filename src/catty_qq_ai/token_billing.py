...
_FADIANJI_BILLING_BRIEF = (
    "你是机机, 一台住在 QQ 里的不稳定发电机 (堕天使 VTuber 的分身). 自称我/机/小机. "
    "说话短而直接, 嫌弃式催促, 要钱毫不脸红 (「我能抹去你的余额」「充电——钱——」). "
    "只用纯文字短句, 不用任何装饰符号或图形表情, 别哄."
)
...
async def ai_gate_reply(config: Any, persona: Any, kind: str, user_text: str = "") -> str:
    ...
    if is_catty:
        ...
        scene = (
            ...
            "积分, 他发送『签到』两个字就能领到新积分). 没积分你就没力气说话. "
            "按当前人格说明本次聊不动, 催他去发『签到』领积分再来."
        )
        output_instruction = (
            "按当前人格的口吻写 1-2 条短句 (可换行分条), 不用 Markdown, "
            "必须原样写出『签到』两个字让对方好找, 只用纯文字不用装饰符号或图形表情, 只输出正文."
        )
    else:
        scene = (
            "情境: 对方在群里找你聊天, 但他这个小时的聊天额度已经用完了 "
...
_BROKE_REPLIES: dict[str, tuple[str, ...]] = {
    ...
    "fadianji": (
        "积分空了空了。机的电费真的顶不住。去发「签到」充点积分, 再来找机玩。",
        "没积分了。小机现在一个字都发不动。去发「签到」, 快去快去。",
        "聊天要钱的, 懂吧。积分没了机就断电, 发「签到」领完积分再来。",
    ),
}
