...
# 主人 2026-05-30: 大砍 — 旧版 14 个 tag 把衣服/表情/姿势全锁死, 跟 Precise Reference
#   (miaomiaonude 裸图) 打架, 还稀释 NAI v4.5 画风 → 出图廉价糊感. 现在只留**核心身份**
#   (裸参考图锁不死的猫耳/猫尾/发色/瞳色), 衣服/表情/姿势全部交给 deepseek 的场景 caption,
#   画风让 NAI 4.5 自己发挥, 对齐网页 `girl,` 一句话那种干净出图.
CHARACTER_GIRL_TAGS = (
    "1girl, blonde hair, blue eyes, white cat ears, white cat tail"
)
CHARACTER_BOY_TAGS = (
...
DEFAULT_BASE_CAPTION = (
    "{{1boy with 1girl}}, hetero, nsfw, explicit, "
    "kissing, hugging, on bed, indoor"
)
DEFAULT_NEGATIVE = (
...
async def _compose_base_caption(
    ...
    **不要 AI 写角色锁** (1girl/1boy/白发/黑影等), 那些由 characterPrompts 锁定。
    ...
    # 强补关键 tag — 仅作功能性兜底 (双人构图 + NSFW 锁 + 该档性行为), deepseek 漏了才补。
    # 主人 2026-05-30: 精简提权 — 旧版每条都 {{}}/{{{}}} 提权, 跟精简 caption 的初衷冲突,
    #   反而把画风压糊. 现在双人锁保留单层 {{}} (功能上需要 NAI 画出黑影主人), nsfw/explicit
    #   /性行为 tag 全用 plain (不提权), 让 NAI 4.5 自己平衡画面.
    lowered = cleaned.lower()
    extra: list[str] = []
    # 双人锁放最前 (NAI v4.5 越靠前权重越大) — 这是唯一保留提权的, 否则黑影主人渲不出来
    if "1boy with 1girl" not in lowered and "hetero couple" not in lowered:
        extra.append("{{1boy with 1girl}}")
...
async def maybe_generate_image(
    *,
    ...
    if miao_dir is not None:
        # 主人 2026-05-30: NSFW 路径只用裸图当参考(miaomiaonude.png), 着装图给普通画像用.
        for ref_name in ("miaomiaonude.png",):
    ...
    # 主人 2026-05-30: 跨境 socks5 抖动 → 偶发超时(curl error)/200 后 body 被截断(坏 zip).
    # 加自动重试. 可重试: 网络错/超时(status==0)、5xx、429、坏 zip/空 zip.
    # 不重试: 4xx(除 429) —— payload/auth 真错, 重试也白费.
    max_attempts = int(getattr(config, "catty_imagegen_nai_max_attempts", 3) or 3)
    image_bytes = b""
    last_reason = ""
    for attempt in range(1, max_attempts + 1):
        ...
        # 网络错/超时 → 可重试
        if err or status == 0:
            last_reason = f"curl error: {err}"
            logger.warning(
                f"nsfw_imagegen curl error after {elapsed:.1f}s: {err} "
                f"(attempt {attempt}/{max_attempts} user={user_id} turn={turn_count})"
            )
            continue
        # 非 200: 4xx(除 429) 真错不重试, 5xx/429 可重试
        if status != 200:
            retriable = status >= 500 or status == 429
            logger.warning(
                f"nsfw_imagegen status={status} elapsed={elapsed:.1f}s retriable={retriable} "
                f"body={body[:300]!r} (attempt {attempt}/{max_attempts} user={user_id} turn={turn_count})"
            )
            if retriable:
                last_reason = f"status={status}"
                continue
            return None
        # 200: 解 zip; 坏 zip / 空 zip 多半是 body 被截断 → 可重试
        try:
            ...
            if not names:
                last_reason = "zip 是空的"
                logger.warning(
                    f"nsfw_imagegen: zip 是空的 (attempt {attempt}/{max_attempts} user={user_id})"
                )
                continue
            image_bytes = zf.read(names[0])
        except (zipfile.BadZipFile, OSError, ValueError) as exc:
            last_reason = f"zip 解包失败 {exc}"
            logger.warning(
                f"nsfw_imagegen: zip 解包失败 {exc} (body_len={len(body)} "
                f"attempt {attempt}/{max_attempts} user={user_id})"
            )
            continue
        if not image_bytes:
            last_reason = "image_bytes 为空"
            continue
        break  # 成功拿到图, 跳出重试
    else:
        # 所有 attempt 都 continue 了(没 break) → 彻底失败
        logger.warning(
            f"nsfw_imagegen: {max_attempts} 次尝试都失败, 放弃 "
            f"(last={last_reason} user={user_id} turn={turn_count})"
        )
        return None
