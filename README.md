...
游戏本地记忆：程序内置了《星痕共鸣》/Blue Protocol: Star Resonance 和《卡拉彼丘》/Strinova 的轻量本地语料记忆；本轮输入包含相关关键词时，会把这份本地记忆提供给主 AI。`game_context.star_resonance_group_ids` 和 `game_context.strinova_group_ids` 还能把指定群配置为对应游戏主题群，即使本轮消息没直接点名游戏，也会提供短背景。若用户要最新赛季、强度榜、活动、兑换码或版本更新，仍建议触发联网搜索。
...
`game_context.star_resonance_group_ids` 和 `game_context.strinova_group_ids` 用来标记游戏主题群，只影响本地游戏背景注入，不影响特别关心、总结范围或普通群插话逻辑。星痕共鸣背景会帮助模型理解职业、副本、装备、活动、捏脸、载具和公会等话题；卡拉彼丘背景会帮助模型理解弦化、超弦体、枪械、地图、爆破、护送、阵营和米雪儿人格背景等话题。
...
默认会把本地表情库候选交给主 AI；普通轻松聊天会优先让 AI 自己挑一张表情包，若 AI 没输出表情标记，程序也会按 `emoji.reply_probability` 尝试自动补一张本地表情。`emojis/manifest.json` 记录表情含义和标签；`emoji.dir` 下未登记的默认表情图片会在扫描时自动补入 manifest，`emoji.download_dir` 下的图片则在被匹配、收养或下载时登记。如果 AI 明确点名了一个本地没有的表情，程序会先尝试收养下载目录里的匹配文件，再用图片搜索下载候选到 `emoji.download_dir`；下载/收养后的新表情会交给 `vision` 识图模型生成 `meaning/tags` 并写回 manifest，方便后续命中。
...
| `game_context.star_resonance_group_ids` | `[]` | 这些群默认按《星痕共鸣》主题群注入本地游戏背景 |
| `game_context.strinova_group_ids` | `[]` | 这些群默认按《卡拉彼丘》/Strinova 主题群注入本地游戏背景 |
