...
class TrainingDashboardTests(unittest.TestCase):
    ...
    def test_model_test_prompt_and_score_file(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("敏感/超限边界", str(messages[1]["content"]))
            self.assertIn("回复前请在心里完成一次自检", str(messages[2]["content"]))
            self.assertIn("主线程模拟请求", str(messages[4]["content"]))
            self.assertIn("测试窗口模拟记忆", str(messages[5]["content"]))
            self.assertEqual(messages[-1]["content"], "测试一下")
            self.assertEqual(messages[6]["content"], "/no_think")
